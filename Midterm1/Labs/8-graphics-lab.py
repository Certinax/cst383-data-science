...
colors = ['blue' if x == 0 else 'orange' \
              for x in df["am"]]
...
plt.scatter(df[df["am"]==0]["disp"], df[df["am"]==0]["mpg"], color='blue', marker="^")
plt.scatter(df[df["am"]==1]["disp"], df[df["am"]==1]["mpg"], color='orange', marker="^")
